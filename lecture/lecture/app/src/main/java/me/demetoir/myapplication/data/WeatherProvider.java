...
public class WeatherProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            ...
            default: {
                throw new android.database.SQLException("failed delete row " + uri);
            }
        }
    ...
    public int update(
        ...
        switch (match) {
            case WEATHER: {
                //시간 변환
                normalizeDate(values);
                numRows = db.update(WeatherContract.WeatherEntry.TABLE_NAME,
                        values, selection, selectionArgs);
                break;
            }
            case LOCATION: {
                numRows = db.update(WeatherContract.LocationEntry.TABLE_NAME,
                        values, selection, selectionArgs);
                break;
            }
            default: {
                throw new android.database.SQLException("failed update row " + uri);
            }
        }
        if (numRows != 0) {
            getContext().getContentResolver().notifyChange(uri, null);
